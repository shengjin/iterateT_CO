...
for i in range(n_row):
    for j in range(n_col):
        ...
        dv = (freq-RST_freq)/RST_freq*cc + 2.8 
...
plt, axes = plt.subplots(n_row, n_col, figsize=(15, 4.5), dpi=80, sharex=True, sharey=True)
...
for i in range(n_row):
    for j in range(n_col):
        print i,j
        im=axes[i,j].pcolormesh((im_x_au*AU/pc/dpc*206265), (im_y_au*AU/pc/dpc*206265), image_conv.imageJyppix[:,:,(n_col*i+j)*3+jump]*1000.0, vmin=clmin, vmax=clmax)
