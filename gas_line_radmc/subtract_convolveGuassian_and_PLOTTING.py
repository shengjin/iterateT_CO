...
v_min   = image_conv.imageJyppix[:,:,:].min()*1000
v_max   = image_conv.imageJyppix[:,:,:].max()*1000

for i in range(n_row):
    for j in range(n_col):
        if colorlog:
            im=axes[i,j].pcolormesh((im_x_au*AU/pc/dpc*206265), (im_y_au*AU/pc/dpc*206265), image_conv.imageJyppix[:,:,n_col*i+j+jump]*1000.0, cmap='jet', norm=LogNorm(vmin=v_min, vmax=v_max))
            #im=axes[i,j].pcolormesh((im_x_au*AU/pc/dpc*206265), (im_y_au*AU/pc/dpc*206265), image_conv.imageJyppix[:,:,n_col*i+j+jump]*1000.0, cmap='jet', norm=LogNorm(linthresh=0.03, linscale=0.03, vmin=v_min, vmax=v_max))
        else:
            im=axes[i,j].pcolormesh((im_x_au*AU/pc/dpc*206265), (im_y_au*AU/pc/dpc*206265), image_conv.imageJyppix[:,:,n_col*i+j+jump]*1000, cmap='jet', vmin=v_min, vmax=v_max)
        axes[i,j].tick_params(axis='both', which='major', labelsize=8)
        axes[i,j].set_xlim([-4, 4])
        axes[i,j].set_ylim([-4, 4])
        #tick.label.set_fontsize(14) 
        #axes[i,j].axis([(im_x_au.min()-axisadd*AU/pc/dpc*206265), (im_x_au.max()+axisadd)*AU/pc/dpc*206265, im_y_au.min()-axisadd, im_y_au.max()+axisadd])
        #axes[i,j].set_yticks( [-600, -400, -200, 0, 200, 400, 600] )
        #axes[i,j].set_xticks( [-600, -400, -200, 0, 200, 400, 600] )
        #axes[i,j].set_xticklabels(['-600', '-400', '-200', '0', '200', '400', '600'],rotation=90)
        if j==0:
